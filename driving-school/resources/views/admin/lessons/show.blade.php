...
<div class="card max-w-xl p-5">
    <dl class="space-y-3 text-sm">
        @foreach ([
            ...
            __('Instructor') => $lesson->instructor?->full_name,
            ...
            __('Recorded By') => $lesson->recorder?->name ?? '—',
    ...
    <div class="mt-5 flex gap-2">
        <a href="{{ route('admin.lessons.edit', $lesson) }}" class="btn-primary btn-sm">{{ __('Edit') }}</a>
        <a href="{{ route('admin.lessons.index') }}" class="btn-secondary btn-sm">{{ __('Back') }}</a>
    </div>
</div>
@endsection
