@extends('layouts.app')
@section('title', $attendance->exists ? __('Edit Attendance') : __('Record Attendance'))
@section('heading', $attendance->exists ? __('Edit Attendance') : __('Record Attendance'))
...
<form method="POST" action="{{ $attendance->exists ? route('admin.attendance.update', $attendance) : route('admin.attendance.store') }}">
        ...
        <div class="grid gap-4 p-5 sm:grid-cols-2">
                ...
                <select id="student_id" name="student_id" required class="input @error('student_id') input-error @enderror">
                    ...
                    @foreach ($students as $student)
                        <option value="{{ $student->id }}" @selected(old('student_id', $attendance->student_id) == $student->id)>
                            {{ $student->full_name }} ({{ $student->student_number }}) — {{ $student->currentInstructor?->full_name ?? __('Unassigned') }}
                        </option>
                    @endforeach
                </select>
            </x-field>
            ...
            <x-field name="attendance_date" :label="__('Date')" required>
                <input id="attendance_date" name="attendance_date" type="date" required max="{{ now()->toDateString() }}"
                       value="{{ old('attendance_date', $attendance->attendance_date instanceof \Illuminate\Support\Carbon ? $attendance->attendance_date->format('Y-m-d') : $attendance->attendance_date) }}"
    ...
    <div class="mt-6 flex gap-2">
        ...
        <a href="{{ route('admin.attendance.index') }}" class="btn-secondary">{{ __('Cancel') }}</a>
    </div>
</form>
@endsection
