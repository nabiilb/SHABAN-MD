...
@section('content')
<x-page-header :title="__('My Lessons')" :subtitle="__(':count records', ['count' => $lessons->total()])">
    <x-slot:actions>
        <a href="{{ route('instructor.lessons.create') }}" class="btn-primary"><x-icon name="plus" class="h-4 w-4" /> {{ __('Add Lesson') }}</a>
    </x-slot:actions>
</x-page-header>
...
<div class="card mb-4 p-4">
    <form method="GET" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('instructor.lessons.index') }}" class="btn-secondary">{{ __('Reset') }}</a>
        </div>
    </form>
</div>
...
<div class="card">
    <div class="table-wrap">
        <table class="table">
            <thead><tr><th>{{ __('Date') }}</th><th>{{ __('Student') }}</th><th>{{ __('Lesson Type') }}</th><th>{{ __('Topic') }}</th>
                <th>{{ __('Vehicle') }}</th><th>{{ __('Duration') }}</th><th>{{ __('Performance') }}</th>
                <th>{{ __('Status') }}</th><th class="text-right">{{ __('Actions') }}</th></tr></thead>
            <tbody>
                @forelse ($lessons as $lesson)
                    <tr>
                        ...
                        <td class="whitespace-nowrap text-right">
                            @can('update', $lesson)
                                <a href="{{ route('instructor.lessons.edit', $lesson) }}" class="btn-ghost btn-sm">{{ __('Edit') }}</a>
                                <x-delete-form :action="route('instructor.lessons.destroy', $lesson)" />
                            @endcan
                        </td>
                    </tr>
                @empty
